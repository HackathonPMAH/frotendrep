...
const PerformanceMetrics = ({ responseTimes, exitGateUsage, isLoading }: Props) => {
  const barData = responseTimes?.length ? responseTimes : FALLBACK_BAR;
  const pieData = exitGateUsage?.length ? exitGateUsage : FALLBACK_PIE;

  return (
    <div className={`grid grid-cols-2 gap-6 transition-opacity ${isLoading ? "opacity-75" : "opacity-100"}`}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={barData} layout="vertical">
            ...
            <YAxis
              dataKey="name"
              type="category"
              tick={{ fill: "#6b5f80", fontSize: 10, fontFamily: "JetBrains Mono" }}
              axisLine={false}
              tickLine={false}
              width={110}
            />
