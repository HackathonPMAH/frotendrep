...
const IncidentTable = () => (
  <div className="glass-panel rounded-xl p-5">
    <div className="flex items-center justify-between mb-4">
      ...
      <span className="font-mono text-[10px] bg-neon-red/15 text-neon-red px-2 py-0.5 rounded-full">{INCIDENTS.length} records</span>
    </div>
        ...
        <thead>
          <tr className="border-b border-border/40">
            {["TIMESTAMP", "ZONE", "LEVEL", "MESSAGE"].map((h) => (
              <th key={h} className="text-left font-mono text-[10px] text-muted-foreground pb-2 pr-4">{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {INCIDENTS.map((inc, i) => (
            <tr key={i} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
              ...
              <td className="py-2 pr-4">
                <span className="font-mono text-[9px] font-bold bg-neon-red/15 text-neon-red border border-neon-red/30 rounded px-1.5 py-0.5">
                  {inc.level}
                </span>
              </td>
